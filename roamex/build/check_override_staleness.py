...
Every file under ``roamex/chromium_src/`` shadows an upstream Chromium file. This tool records a
sha256 of the *pristine* upstream content — read with ``git show <pin>:<path>`` at the revision named
in ``roamex/build/CHROMIUM_PIN``, never from the (patched) working tree — into
...
def main():
    ...
    parser.add_argument("--overlay", type=pathlib.Path,
                        ...
                        help="overlay root containing chromium_src/ (default: roamex/)")
