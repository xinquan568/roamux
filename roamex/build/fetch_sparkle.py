...
extracts it into roamex/third_party/sparkle/ (gitignored except README.md).
Idempotent: an existing Sparkle.framework at the destination is a no-op
unless --force. Builds with roamex_enable_sparkle=true fail loudly when the
framework is absent and name this script.

Usage:
  roamex/build/fetch_sparkle.py [--dest DIR] [--force]
  # test/offline overrides:
  roamex/build/fetch_sparkle.py --archive /path/to/archive.tar.xz \
      [--expected-sha256 HEX]
"""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="roamex-sparkle-") as tmp:
        if args.archive:
