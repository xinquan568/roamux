...
Mirror the upstream path exactly, e.g. `roamex/chromium_src/chrome/common/chrome_isolated_world_ids.h`
(the roam-2 sample, a full upstream copy + one inert marker).

## How the redirect works (v1)

`patches/0002-chromium-src-include-redirect.patch` prepends `//roamex/chromium_src` to GN's
...
```sh
python3 roamex/build/check_override_staleness.py --chromium-src ~/chromium/src            # gate
python3 roamex/build/check_override_staleness.py --chromium-src ~/chromium/src --update   # after a reviewed uprev
```
...
additive `//roamex` code; record every override here AND in the §12.2 inventory.
