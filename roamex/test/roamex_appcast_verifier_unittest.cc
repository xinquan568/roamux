...
TEST(RoamexAppcastVerifierTest, FixtureSignatureVerifies) {
  std::string artifact;
  ASSERT_TRUE(base::ReadFileToString(
      FixtureDir().AppendASCII("Roamex-99.0.0.zip"), &artifact));
  EXPECT_TRUE(roamex::app::VerifyAppcastSignature(
      base::as_byte_span(artifact), ReadTrimmed("artifact_signature.b64"),
      ReadTrimmed("test_public_ed_key.b64")));
}

TEST(RoamexAppcastVerifierTest, TamperedArtifactRejected) {
  std::string artifact;
  ASSERT_TRUE(base::ReadFileToString(
      FixtureDir().AppendASCII("Roamex-99.0.0.zip"), &artifact));
  artifact[0] ^= 0x01;
  EXPECT_FALSE(roamex::app::VerifyAppcastSignature(
      base::as_byte_span(artifact), ReadTrimmed("artifact_signature.b64"),
      ReadTrimmed("test_public_ed_key.b64")));
}

TEST(RoamexAppcastVerifierTest, EmptyOrGarbageSignatureRejected) {
...
TEST(RoamexAppcastVerifierTest, WrongKeyRejectedAndInProcessRoundTrip) {
  ...
  EXPECT_TRUE(roamex::app::VerifyAppcastSignature(base::as_byte_span(payload),
                                                  sig_b64, pub_b64));
