...
class RoamexClearBrowsingDataTest : public InProcessBrowserTest {
 public:
  RoamexClearBrowsingDataTest() {
...
IN_PROC_BROWSER_TEST_F(RoamexClearBrowsingDataTest,
...
IN_PROC_BROWSER_TEST_F(RoamexClearBrowsingDataTest, ClearFutureRangeIsNoOp) {
  SeedThreeTabs();
...
// (and thus the roamex hook) is skipped, so the journal is NOT wiped even
// though the mask requests it (DATA_TYPE_NO_CHECKS avoids the upstream policy
// DCHECK).
IN_PROC_BROWSER_TEST_F(RoamexClearBrowsingDataTest, PolicyBlockedKeepsJournal) {
  SeedThreeTabs();
  const size_t before = ReadVisits(browser()->profile()).size();
  ASSERT_GE(before, 3u);
