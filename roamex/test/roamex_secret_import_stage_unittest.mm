...
class RoamexSecretImportStageTest : public testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(RoamexSecretImportStageTest, ImportsDecryptedPasswords) {
  {
...
TEST_F(RoamexSecretImportStageTest, DeclinedKeychainImportsNothingReported) {
...
TEST_F(RoamexSecretImportStageTest, ImportsFirstPartyCookieWithV24Strip) {
  {
    sql::Database db(kTag);
...
TEST_F(RoamexSecretImportStageTest, PartitionedCookieSkipped) {
  {
    sql::Database db(kTag);
    ASSERT_TRUE(db.Open(source().Append(FILE_PATH_LITERAL("Cookies"))));
    ASSERT_TRUE(db.Execute(
