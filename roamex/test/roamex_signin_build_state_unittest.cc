...
using roamex::signin::GetBuildState;
using roamex::signin::GetBuildStateForInputs;
using roamex::signin::IsSigninInert;

// Decision table (SS7.3/SS12.3): keys are necessary but not sufficient.
TEST(RoamexSigninBuildStateTest, DecisionTableCoversAllInputCombinations) {
...
TEST(RoamexSigninBuildStateTest, ReferenceBuildAuthorizationFlagIsFalse) {
  constexpr bool kAuthorized = BUILDFLAG(ROAMEX_SIGNIN_AUTHORIZED_BUILD);
  EXPECT_FALSE(kAuthorized)
      << "the reference build must never assert Google authorization "
         "(roamex_signin_authorized_build is a distributor-only GN arg)";
}
...
TEST(RoamexSigninBuildStateTest, ReferenceBuildIsKeylessAndInert) {
