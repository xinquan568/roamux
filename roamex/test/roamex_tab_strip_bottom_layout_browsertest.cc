...
class RoamexTabStripBottomLayoutTest : public InProcessBrowserTest {
 public:
  RoamexTabStripBottomLayoutTest() {
...
IN_PROC_BROWSER_TEST_F(RoamexTabStripBottomLayoutTest,
...
class RoamexTabStripBottomLayoutFlagOffTest : public InProcessBrowserTest {
 public:
  RoamexTabStripBottomLayoutFlagOffTest() {
...
IN_PROC_BROWSER_TEST_F(RoamexTabStripBottomLayoutFlagOffTest,
...
// Coexistence rule D1: while upstream vertical tabs are displayed, the roamex
// placement is structurally inert — the vertical strip owns the layout and the
// horizontal strip stays hidden regardless of roamex.tabs.strip_position.
class RoamexTabStripCoexistenceTest : public InProcessBrowserTest {
 public:
  RoamexTabStripCoexistenceTest() {
...
IN_PROC_BROWSER_TEST_F(RoamexTabStripCoexistenceTest,
                       UpstreamVerticalTabsWinOverRoamexBottom) {
  BrowserView* bv = BrowserView::GetBrowserViewForBrowser(browser());
  PrefService* prefs = browser()->profile()->GetPrefs();
  prefs->SetBoolean(::prefs::kVerticalTabsEnabled, true);
  prefs->SetInteger(prefs::kTabStripPosition, 1);  // roamex bottom.
  base::RunLoop().RunUntilIdle();
  bv->DeprecatedLayoutImmediately();
