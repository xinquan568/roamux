...
BASE_DECLARE_FEATURE(
    kTabStripPosition);             // E1 — configurable tab-strip position
BASE_DECLARE_FEATURE(kInitialUrl);  // E2 — per-tab initial URL
BASE_DECLARE_FEATURE(kEdgeImport);  // E3 — Microsoft Edge import
BASE_DECLARE_FEATURE(
    kBookmarkSubfolderGroups);  // roam-208 — bookmarks: subfolders as tab
                                // groups (roam-190; SHIPS ENABLED, kill-switch)
BASE_DECLARE_FEATURE(kTabVisitNav);         // E4 — tab visit-order navigation
BASE_DECLARE_FEATURE(kBraveStyleProfiles);  // E5 — Brave-style profiles +
                                            // hidden optional sign-in
BASE_DECLARE_FEATURE(
...
BASE_DECLARE_FEATURE(
    kRoamuxExternalOpenProfile);  // roam-213 — designated profile for external
                                  // opens (Finder files / links from other
                                  // apps)
BASE_DECLARE_FEATURE(
    kTabStripToggleShortcut);  // roam-214 — pin/peek + collapse/expand shortcut
                               // for the vertical tab strip
BASE_DECLARE_FEATURE(
    kRefreshAllInitialUrls);  // roam-269 — Ctrl+Opt+Cmd+R refreshes every tab
...
// as raw integers into the pure scheduler's Params::FromMilliseconds, which
// owns validation — the scheduler itself is FeatureParam-unaware so it can stay
// in the //roamux/browser/tabs source_set with no //chrome deps.
//   --enable-features=RoamuxRefreshAllInitialUrls:interval_ms/8000
extern const base::FeatureParam<int> kRefreshAllInitialUrlsIntervalMs;
extern const base::FeatureParam<int> kRefreshAllInitialUrlsMinSpacingMs;
