...
// caption (traffic-light) exclusion is preserved with a bottom strip, Roamux
...
IN_PROC_BROWSER_TEST_F(RoamuxFrameMatrixTest,
...
class RoamuxRtlPlacementTest : public RoamuxFrameMatrixTest {
 public:
  void SetUpCommandLine(base::CommandLine* command_line) override {
    RoamuxFrameMatrixTest::SetUpCommandLine(command_line);
    command_line->AppendSwitchASCII(::switches::kForceUIDirection,
                                    ::switches::kForceDirectionRTL);
  }
};

IN_PROC_BROWSER_TEST_F(RoamuxRtlPlacementTest,
...
IN_PROC_BROWSER_TEST_F(RoamuxRtlPlacementTest, BottomPlacementFullWidthInRtl) {
  SetPlacementAndLayout(1);  // kBottom.
  views::View* strip =
      FindViewByClassName(browser_view(), "HorizontalTabStripRegionView");
  ASSERT_NE(nullptr, strip);
...
IN_PROC_BROWSER_TEST_F(RoamuxFrameMatrixTest, FullscreenMatrixIsSane) {
...
IN_PROC_BROWSER_TEST_F(RoamuxFrameMatrixTest, AppWindowIgnoresPlacement) {
  ...
  Browser* app_browser = Browser::Create(Browser::CreateParams::CreateForApp(
      "roamux_test_app", /*trusted_source=*/true, gfx::Rect(),
  ...
  // App windows have no tab strip; the roamux placement must not conjure one
