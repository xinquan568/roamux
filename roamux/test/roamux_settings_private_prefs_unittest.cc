...
// roamux.tabs.strip_position must be exposed through the settings_private
...
class RoamuxSettingsPrivatePrefsTest : public testing::Test {
 protected:
...
TEST_F(RoamuxSettingsPrivatePrefsTest, SigninOptInIsAllowlistedAsBoolean) {
  std::optional<settings_api::PrefObject> pref =
      prefs_util_.GetPref(roamux::prefs::kSigninOptionalEntryPoint);
  ASSERT_TRUE(pref.has_value());
  EXPECT_EQ(settings_api::PrefType::kBoolean, pref->type);
  EXPECT_EQ(false, pref->value->GetBool());
}

TEST_F(RoamuxSettingsPrivatePrefsTest, SigninOptInRoundTripsThroughSetPref) {
  base::Value on(true);
  EXPECT_EQ(extensions::settings_private::SetPrefResult::SUCCESS,
            prefs_util_.SetPref(roamux::prefs::kSigninOptionalEntryPoint, &on));
...
TEST_F(RoamuxSettingsPrivatePrefsTest, GetPrefExposesTheRegisteredDefault) {
  std::optional<settings_api::PrefObject> pref =
...
TEST_F(RoamuxSettingsPrivatePrefsTest, SetPrefRoundTripsAndReadsLive) {
  base::Value two(2);
  EXPECT_EQ(extensions::settings_private::SetPrefResult::SUCCESS,
