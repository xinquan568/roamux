...
@interface RoamuxTestUserDriver : NSObject <SPUUserDriver>
...
@implementation RoamuxTestUserDriver
...
- (void)showUpdaterError:(NSError*)error
         acknowledgement:(void (^)(void))acknowledgement {
  NSLog(@"roamux harness updater error: %@", error);
  self.updaterError = error;
  self.finished = YES;
  acknowledgement();
}
...
class RoamuxSparkleFeedTest : public testing::Test {
 protected:
  ...
  void RunCheck(const std::string& appcast_name) {
    ...
    server.RegisterRequestHandler(base::BindRepeating(
        &RoamuxSparkleFeedTest::Serve, base::Unretained(this)));
    ...
    ASSERT_TRUE(base::ReadFileToString(
        FixtureDir().AppendASCII("Roamux-99.0.0.zip"), &artifact_));
...
<plist version="1.0"><dict>
  ...
  <key>SUEnableAutomaticChecks</key><false/>
    ...
    @autoreleasepool {
      ...
      driver_ = [[RoamuxTestUserDriver alloc] init];
...
// no updater error.
TEST_F(RoamuxSparkleFeedTest, SignedItemPassesSignatureValidation) {
...
TEST_F(RoamuxSparkleFeedTest, TamperedItemRejectedAtSignatureGate) {
